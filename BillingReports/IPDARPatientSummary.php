...
<table width='100%' border='0' cellspacing='0' cellpadding='0'>
  <tr>
    <td height='30'><div align='left' class='arial15blackbold'>IPD A/R - PATIENT SUMMARY</div></td>
  </tr>
  <tr>
...
$totalall=0;
/*$asql=mysql_query("SELECT rd.registrationNo, rd.type, rd.discount AS disc, rd.dateRegistered FROM patientCharges pc, registrationDetails rd WHERE pc.registrationNo=rd.registrationNo AND rd.type='OPD' AND pc.status NOT LIKE 'DELETED_%%%%' AND (rd.dateRegistered BETWEEN '$fdate' AND '$tdate') GROUP BY rd.registrationNo ORDER BY rd.registrationNo");
...
$bsql=mysql_query("SELECT itemNo, sellingPrice, quantity, discount, total, cashUnpaid, phic, company, cashPaid, cashPaidFromBalance, dateCharge, amountPaidFromCreditCard, orNo FROM patientCharges WHERE registrationNo='$registrationNo' AND title NOT LIKE 'PROFESSIONAL FEE' AND status NOT LIKE 'DELETED_%%%%'");
...
$csql=mysql_query("SELECT itemNo, sellingPrice, quantity, discount, total, cashUnpaid, phic, company, cashPaid, dateCharge, amountPaidFromCreditCard, orNo, doctorsPF FROM patientCharges WHERE registrationNo='$registrationNo' AND status NOT LIKE 'DELETED_%%%%' AND title='PROFESSIONAL FEE'");
...
$totalall+=($totcashUnpaid+$pfcashUnpaid);
...
echo "
      <tr>
        ...
        <td class='table1Bottom'><a href='../COCONUT/patientProfile/SOAoption/newSOA/newDetailed.php?registrationNo=$registrationNo&username=$username&show=try' class='astyle' target='_blank'><div align='right' class='arial14black'>&nbsp;".number_format((($dtotcashUnpaid+$epfcashUnpaid)-($finalzamountPaidCashhb+$finalzamountPaidCredithb+$finalzamountPaidCashd+$finalzamountPaidCreditd)),2,'.',',')."&nbsp;</div></a></td>
      </tr>
";
}
}
