...
<table width='100%' border='0' cellspacing='0' cellpadding='0'>
  <tr>
    <td><div align='left'><img src='../../COCONUT/myImages/mendero.png' height='10%' width='30%' /></div></td>
  </tr>
  <tr>
...
$asubtotCP=0;
$asql=mysql_query("SELECT registrationNo, orNO, datePaid, timePaid FROM patientCharges WHERE status='PAID' AND orNO NOT LIKE '' AND (datePaid BETWEEN '$fdate' AND '$tdate') AND (timePaid BETWEEN '$ftime' AND '$ttime') AND paidVia='Cash' GROUP BY orNO,registrationNo ORDER BY datePaid,timePaid");
...
$btotCP=0;
$bsql=mysql_query("SELECT cashPaid, title, paidBy, doctorsPF FROM patientCharges WHERE registrationNo='$aregistrationNo' AND orNO='$aorNO'");
...
if($btitle=='PROFESSIONAL FEE'){$btotCP+=($bcashPaid+$bdoctorsPF);}else{$btotCP+=$bcashPaid;}
...
echo "
      <tr>
        ...
        <td><div align='center' class='style4'>$aorNO</div></td>
...
$csubtotCP=0;
$csql=mysql_query("SELECT registrationNo, orNOFromBalance, datePaidFromBalance, timePaidFromBalance FROM patientCharges WHERE orNOFromBalance NOT LIKE '' AND (datePaidFromBalance BETWEEN '$fdate' AND '$tdate') AND (timePaidFromBalance BETWEEN '$ftime' AND '$ttime') GROUP BY orNOFromBalance,registrationNo ORDER BY datePaid,timePaid");
...
$corNOFromBalance=$cfetch['orNOFromBalance'];
...
$dtotCP=0;
$dsql=mysql_query("SELECT cashPaidFromBalance, title, paidBy, doctorsPF FROM patientCharges WHERE registrationNo='$cregistrationNo' AND orNOFromBalance='$corNOFromBalance'");
...
if($dtitle=='PROFESSIONAL FEE'){$dtotCP+=($dcashPaidFromBalance+$ddoctorsPF);}else{$dtotCP+=$dcashPaidFromBalance;}
...
echo "
      <tr>
        ...
        <td><div align='center' class='style4'>$corNOFromBalance</div></td>
...
$esubtotCP=0;
$esql=mysql_query("SELECT registrationNo, orNO, datePaid, timePaid FROM patientCharges WHERE status='PAID' AND orNO NOT LIKE '' AND (datePaid BETWEEN '$fdate' AND '$tdate') AND (timePaid BETWEEN '$ftime' AND '$ttime') AND paidVia='Credit Card' GROUP BY orNO,registrationNo ORDER BY datePaid,timePaid");
...
$ftotCP=0;
$fsql=mysql_query("SELECT title, paidBy, doctorsPF, cardType, creditCardNo, amountPaidFromCreditCard FROM patientCharges WHERE registrationNo='$eregistrationNo' AND orNO='$eorNO'");
...
if($ftitle=='PROFESSIONAL FEE'){$ftotCP+=($famountPaidFromCreditCard+$fdoctorsPF);}else{$ftotCP+=$famountPaidFromCreditCard;}
...
echo "
      <tr>
        ...
        <td><div align='center' class='style4'>$eorNO</div></td>
