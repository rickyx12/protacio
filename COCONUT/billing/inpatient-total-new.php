...
include 'transactionSummaryDetailed/transactionSummary.class.php';
...
<html>
	...
	<body>
							...
							<tbody>
									...
									?>
										<tr>
											<td>
												<? 
													$patientNo = $ro2->selectNow('registrationDetails','patientNo','registrationNo',$registrationNo);
													$lastName = $ro2->selectNow('patientRecord','lastName','patientNo',$patientNo);
													$firstName = $ro2->selectNow('patientRecord','firstName','patientNo',$patientNo);
													echo '<Font size=1>'.$registrationNo.'-'.$lastName.', '.$firstName.'</font>'; 
												?>
											</td>
											...
											<td>
												<?
													$depositCash = $ts->get_inpatient_deposit($registrationNo,'Cash','');
													$depositCreditCard = $ts->get_inpatient_deposit($registrationNo,'Credit Card','');
													$totalDeposit = ( $depositCash + $depositCreditCard );
											...
											<td>
												<?
													 $total = ( $lab + $xray + $utz + $ctscan + $ecg + $erfee + $or + $misc + $spirometry + $meds + $supp + $pf + $room + $cardiac + $pt + $ot + $st + $others - $totalDeposit ); 
													 	$grandTotal += $total;
													 	echo $total;  
												?>
											</td>
											...
											<td>
												<? 
													$unpaid = round( 
														...
														$ro2->selectNow("registrationDetails","excessPF","registrationNo",$registrationNo) -
														$totalDeposit
														,2 
														); 													
													echo $unpaid; 
											...
											<td>
												<? 
													$cash = ($ro4->inpatient_payment_hospitalBill_total($registrationNo,"CASH")); 
													echo $cash; 
													$cashTotal += $cash; 
												?>
											</td>
											...
											<td>
												<? 
													$creditCard = $ro4->inpatient_payment_hospitalBill_total($registrationNo,"Credit Card"); 
													echo $creditCard; 
											...
											<td>
												<? 
													$grandCash = ($ro4->inpatient_payment_hospitalBill_total($registrationNo,"CASH") - $ro4->inpatient_refund_total($registrationNo,"CASH")); 
													echo $grandCash; 
		...
		<form method="get" action="transactionSummary2-titles-new.php">
			...
			<input type="hidden" name="ipd_deposit_less" value="<? echo $lessDeposit ?>">
