...
$ro->transactionSummary_dermaPayments("Credit Card",$date,$date1)
);
...
$ro->transactionSummary_dermaPayments("Cash",$date,$date1)
);
...
echo "<td>Derma</td>";
echo "<td>&nbsp;</td>";
echo "<td align='right'>&nbsp;".number_format($derma,2)."</td>";
...
$opd_creditTotal = ( $totalLab + $totalMed + $totalSup + $totalUTZ + $totalCTSCAN + $totalOR + $totalECG + $totalSPIROMETRY + $totalXRAY + $totalPT + $totalERFEE + $totalCardiacMonitor + $totalClinicRev + $totalPayableMD + $opd_paidBalance + $derma );
