...
<html>
		...
		<script>
			$(document).ready(function(){
				...
				$("#pendingRequest").click(function(){
					open("POST","../requestition/pending-request.php",{module:'NURSING'},"departmentX");
				});

				$("#toDispense").click(function(){
					open("POST","../inventory/to-dispense-dept.php",{inventoryLocation:'NURSING'},'departmentX');
				});

				$("#returnInventory").click(function(){
					open("POST","../inventory/return-inventory.php",{inventoryLocation:'NURSING'},'departmentX');
				});
	...
	<body>
		...
		<div class="menu">
			<ul>
				<li>
					...
					<ul>
						<a href="patient-list.php" target="departmentX">View Patient</a>
				...
				<li>
					...
					<ul>
						...
						<? if( $ro4->count_pending_request("NURSING") > 0 ) { ?>
								<a href="#" id="pendingRequest" >
									Pending Inventory Request 
									<span class="badge">
										<? echo $ro4->count_pending_request("NURSING") ?>
									</span>
								</a>
						<? }else {  ?>
							<a href="#">
						...
						<a href="#" id="returnInventory" >
							Return Inventory
								<? if( $ro4->count_return_inventory('NURSING') > 0 ) { ?>
									<span class="badge">
										<?
											echo $ro4->count_return_inventory('NURSING')
										?>
									</span>
								<? }else { } ?>
						</a>
				...
				<li>
					<a href="#" id="toDispense">
						Dispensing
						<? if( $ro4->count_dept_dispense('NURSING') > 0 ) { ?>
							<span class="badge">
								<?
									echo $ro4->count_dept_dispense('NURSING')
								?>
							</span>
						<? }else { } ?>
					</a>
		...
		<div class="row">
			<iframe src="patient-list.php" style="border:0px; width:100%; height:100%;" name="departmentX" border=1 frameborder=no></iframe>
		</div>
	</body>
</html>