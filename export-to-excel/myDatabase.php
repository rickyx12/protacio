...
public function addNewRegistration($patientNo,$registrationNo,$bloodPressure,$temperature,$height,$weight,$company,$initialDiagnosis,$dateRegistered,$timeRegistered,$branch,$type,$room,$username,$casetype,$limit,$control_dateRegistered,$diet,$pulse,$respiratory,$from) {
...
$sql="INSERT INTO registrationDetails (patientNo,registrationNo,bloodPressure,temperature,height,weight,Company,initialDiagnosis,dateRegistered,timeRegistered,branch,type,room,PIN,registeredBy,casetype,LimitCASH,control_dateRegistered,diet,pulseRate,respiratoryRate,registeredFrom)
VALUES
...
$result = mysql_query("SELECT Category,chargesCode,upper(Description) as Description,(HMO) as sellingPrice,Service,Category FROM availableCharges where Description like '$charges%%%%%' ");
...
if( $this->selectNow("registrationDetails","Company","registrationNo",$registrationNo) != "" && $this->getRegistrationDetails_type() == "IPD" ) {

if( $row['Category'] == "LABORATORY" ) { //15% addon in laboratory
$sellingPrice = $row['sellingPrice'] + ( $row['sellingPrice'] * 0.15 );
}else {
$sellingPrice = $row['sellingPrice'] + ( $row['sellingPrice'] * 0.35 );
}

}else if( $this->getPatientRecord_phic() == "YES" && $this->getRegistrationDetails_type() == "IPD" && $this->selectNow("registrationDetails","Company","registrationNo",$registrationNo) == "" ) {

if( $row['Category'] == "LABORATORY" ) { //15% addon in laboratory
$sellingPrice = $row['sellingPrice'] + ( $row['sellingPrice'] * 0.15 );
}else {
$sellingPrice = $row['sellingPrice'] + ( $row['sellingPrice'] * 0.25 );
}
...
$result = mysql_query("SELECT * FROM patientCharges where registrationNo = '$registrationNo' and title='$title' and status = 'UNPAID' order by dateCharge,timeCharge asc ");
...
if( $nod == "" ) {
$result = mysql_query("SELECT pc.*,rd.type FROM patientCharges pc,registrationDetails rd where rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and pc.inventoryFrom='$title' and pc.dateCharge='$selectedDate' and pc.paidVia ='Cash' and (pc.timeCharge between '$fromTimez' and '$toTimez') and pc.departmentStatus not like 'dispensedBy%%%%%' and pc.status not like 'DELETED_%%%%%%%' order by pc.description asc ");
}else {
$result = mysql_query("SELECT pc.*,rd.type FROM patientCharges pc,registrationDetails rd where rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and pc.inventoryFrom='$title' and pc.dateCharge='$selectedDate' and pc.paidVia ='Cash' and (pc.timeCharge between '$fromTimez' and '$toTimez') and pc.departmentStatus not like 'dispensedBy%%%%%' and pc.status not like 'DELETED_%%%%%%%' and pc.chargeBy = '$nod'  order by pc.description asc ");
}
...
public function addNewMedicine($description,$generic,$unitcost,$quantity,$expiration,$addedBy,$dateAdded,$timeAdded,$inventoryLocation,$inventoryType,$branch,$transition,$remarks,$preparation,$phic,$added,$criticalLevel,$supplier,$begCapital,$begQTY,$suppliesUNITCOST,$autoDispense) {
...
$sql="INSERT INTO inventory (description,genericName,unitcost,quantity,expiration,addedBy,dateAdded,timeAdded,inventoryLocation,inventoryType,branch,transition,remarks,preparation,phic,Added,criticalLevel,supplier,beginningCapital,beginningQTY,suppliesUNITCOST,autoDispense)
VALUES
('$description','$generic','$unitcost','$quantity','$expiration','$addedBy','$dateAdded','$timeAdded','$inventoryLocation','$inventoryType','$branch','$transition','$remarks','$preparation','$phic','$added','$criticalLevel','$supplier','$begCapital','$begQTY','$suppliesUNITCOST','$autoDispense')";
...
if( $inventoryType == "medicine" ) {
$sql="INSERT INTO inventory (description,genericName,unitcost,quantity,expiration,addedBy,dateAdded,timeAdded,inventoryLocation,inventoryType,branch,transition,remarks,Added,autoDispense,from_inventoryCode,beginningQTY)
VALUES
('$description','$generic','$unitcost','$quantity','$expiration','$addedBy','$dateAdded','$timeAdded','$inventoryLocation','$inventoryType','$branch','$transition','$remarks','$price','yes','$inventoryCode','$quantity')";
}else {
$sql="INSERT INTO inventory (description,genericName,suppliesUNITCOST,quantity,expiration,addedBy,dateAdded,timeAdded,inventoryLocation,inventoryType,branch,transition,remarks,unitcost,autoDispense,from_inventoryCode,beginningQTY)
VALUES
('$description','$generic','$unitcost','$quantity','$expiration','$addedBy','$dateAdded','$timeAdded','$inventoryLocation','$inventoryType','$branch','$transition','$remarks','$price','yes','$inventoryCode','$quantity')";
}
...
if( $this->getRegistrationDetails_company() != "" && $this->getRegistrationDetails_type() != "OPD" ) {
echo "<td>&nbsp;".number_format($priceOption[1] + ($priceOption[1] * 0.20) ,2)."&nbsp;</td>"; 
...
echo "<td><font size=2 color=red>Confidential</font></td>";
$this->med_sp = ($priceOption[1] + ($priceOption[1] * 0.35)); 
//$this->med_sp = $priceOption[1];
}
...
$result = mysql_query("SELECT inventoryCode,description,unitcost,quantity FROM inventory WHERE $searchBy like '%%%%%%%%$searchDesc%%%%%%%' and inventoryType = 'supplies' and inventoryLocation = '$searchFrom' and quantity > 0 and status not like 'DELETED_%%%%%%' order by $searchBy asc ");
...
if( $this->getRegistrationDetails_company() != "" ) { 
echo "<td>&nbsp;".($row['unitcost'] + ( $row['unitcost'] * 0.35 ) )."&nbsp;</td>";
$this->sup_sp = ($row['unitcost'] + ($row['unitcost'] * 0.35 ) );
}else if( $this->getPatientRecord_phic() == "YES" ) { 
echo "<td>&nbsp;".($row['unitcost'] + ($row['unitcost'] * 0.25 ) )."&nbsp;</td>";
$this->sup_sp = ($row['unitcost'] + ($row['unitcost'] * 0.25 ) );
}else {
...
}else if( $this->selectNow("registeredUser","module","username",$row['chargeBy']) == "ER" ) {
echo "<td>&nbsp;<a href='http://".$this->getMyUrl()."/Department/patientDepartmentProfile.php?registrationNo=$row[registrationNo]&module=$module&month=$m&day=$d&year=$y&fromTime_hour=$fromTime_hour&fromTime_minutes=$fromTime_minutes&fromTime_seconds=$fromTime_seconds&toTime_hour=$toTime_hour&toTime_minutes=$toTime_minutes&toTime_seconds=$toTime_seconds&username=$username&module=$module&nod=' target='patientCharges'><font size='2' color=red>".$row['lastName']." ".$row['firstName']."</font></a>&nbsp;</td>";
}
else {
...
$result = mysql_query("SELECT upper(pr.completeName) as completeName,pc.description,pc.sellingPrice,pc.quantity,pc.discount,pc.total,pc.cashUnpaid,pc.cashPaid,pc.chargeBy FROM patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pc.registrationNo and pc.dateCharge = '$dateSelected' and (pc.timeCharge between '$fromTimez' and '$toTimez') and title='$module' and pc.status not like 'DELETED_%%%%%' group by pc.itemNo order by completeName asc ");
...
if( $this->selectNow("registeredUser","module","username",$username) == "ADMIN" ) {
$result = mysql_query("SELECT rd.registrationNo,pp.paidVia,upper(pr.completeName) as completeName,pp.paymentFor,pp.paidBy,pp.datePaid,pp.amountPaid,pp.pf,pp.admitting,pp.receiptType FROM patientPayment pp,patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and pp.registrationNo = rd.registrationNo and rd.registrationNo = pc.registrationNo and pp.datePaid = '$dateSelected' and pp.paidBy = '$username' and (pp.timePaid between '$fromTimez' and '$toTimez') and pp.paymentFor != 'REFUND' group by paymentNo order by completeName asc ");
}else {
...
if( $show == "All" ) {
$result = mysql_query("SELECT pc.description,rd.registrationNo,upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,pc.service,pc.sellingPrice from patientRecord pr,registrationDetails rd,patientCharges pc WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pc.registrationNo and (pc.datePaid between '$date' and '$date1') and title = 'PROFESSIONAL FEE' and rd.type='$type' order by pc.description,pr.lastName asc    ");
}else {
...
if($show == "All") {
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and pc.registrationNo = rd.registrationNo and pc.status = 'UNPAID' order by title asc  ");
}

else if($show == "try") {
$result = mysql_query("SELECT pc.chargesCode,pc.quantity,pc.description,pc.quantity,pc.sellingPrice,pc.dateCharge,pc.discount,pc.total,pc.cashPaid,pc.company,pc.phic,pc.cashUnpaid,pc.title,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and pc.registrationNo = rd.registrationNo and pc.status = 'UNPAID' order by pc.title asc  ");
}
...
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and title='$data1' and pc.status = 'UNPAID' order by description asc  ");
}else if($show == "phic") {
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and phic > 0 and pc.status = 'UNPAID' order by description asc  ");
}else if($show == "company") {
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and pc.company > 0 and pc.status  = 'UNPAID' order by pc.description asc  ");
...
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and cashUnpaid > 0 and pc.status = 'UNPAID' order by description asc  ");
}else if($show == "selected") {
$result = mysql_query("SELECT pc.*,rd.* from patientCharges pc,registrationDetails rd WHERE rd.registrationNo = '$registrationNo' and rd.registrationNo = pc.registrationNo and itemNo='$data1' and pc.status = 'UNPAID' order by description asc  ");
}else {
echo "";
...
$result = mysql_query("SELECT paymentFor,( amountPaid + pf + admitting ) as total,orNo FROM patientPayment WHERE registrationNo = '$registrationNo' and paymentFor != 'REFUND' ");
...
$result = mysql_query("SELECT upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,upper(pr.middleName) as middleName,rd.*,pr.Age,pr.Gender,pr.phic FROM patientRecord pr,registrationDetails rd WHERE pr.patientNo = rd.patientNo and (dateRegistered between '$fromRegistered' and '$toRegistered') and rd.type='$type' and rd.registeredFrom='$dept' order by timeRegistered asc ");
}
}else {
...
$result = mysql_query("SELECT upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,upper(pr.middleName) as middleName,rd.*,pr.Age,pr.Gender,pr.phic FROM patientRecord pr,registrationDetails rd WHERE pr.patientNo = rd.patientNo and (dateRegistered between '$fromRegistered' and '$toRegistered') and rd.type='$type' order by timeRegistered asc ");
}
...
$result = mysql_query("SELECT description from patientCharges where registrationNo = '$registrationNo' and service like '%$service%'  ");
...
$result = mysql_query("SELECT rd.registrationNo,rd.dateUnregistered,pr.lastName,pr.firstName,pc.sellingPrice,pc.service,pc.dateCharge,pc.timeCharge,pc.cashUnpaid FROM patientCharges pc,registrationDetails rd,patientRecord pr WHERE pc.registrationNo = rd.registrationNo and rd.patientNo = pr.patientNo and pc.description = '$name' and (rd.dateUnregistered between '$datez' and '$datez1') and (rd.type='IPD' or rd.type='OR/DR' or rd.type='ICU') order by pr.lastName   ");
